  ...
  return (
    <div className="max-w-7xl mx-auto px-8 py-16">
      ...
      <header className="mb-20 text-center">
        ...
        <h1 className="text-6xl font-black text-primary tracking-tighter uppercase">Our Brand Portfolio</h1>
        <p className="max-w-2xl mx-auto text-on-surface-variant mt-6">
          We partner with the worlds leading electrical manufacturers to bring you certified, high-performance products for every industrial and domestic requirement.
        </p>
      </header>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-1">
        {brands.map((brand, i) => (
          <motion.div 
            ...
            viewport={{ once: true }}
            className="bg-surface-container-lowest p-12 border border-outline-variant/20 flex flex-col items-center text-center group hover:bg-primary transition-all duration-500"
          >
            <div 
              className="w-24 h-24 flex items-center justify-center text-4xl font-black mb-8 rounded-full border-4 group-hover:bg-white group-hover:border-white transition-all"
              style={{ color: brand.color, borderColor: brand.color }}
            >
              {brand.logo}
            </div>
            <h3 className="text-2xl font-black text-on-surface group-hover:text-white mb-2 tracking-tight">{brand.name}</h3>
            <span className="text-[10px] font-bold uppercase tracking-widest text-secondary group-hover:text-white/80 mb-6">{brand.category}</span>
            <p className="text-sm text-on-surface-variant group-hover:text-white/70 leading-relaxed">{brand.desc}</p>
          </motion.div>
        ))}
      </div>

      <section className="mt-24 bg-surface-container p-12 text-center">
        <h2 className="text-3xl font-black text-primary mb-6 tracking-tight">CANT FIND A SPECIFIC BRAND?</h2>
        ...
        <button 
          ...
          className="bg-primary text-on-primary px-10 py-4 font-bold text-sm uppercase tracking-widest hover:scale-95 transition-transform"
        >
