...
export default function Products() {
  return (
    <div className="max-w-7xl mx-auto px-8 py-16">
      ...
      <header className="mb-16">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-8">
          <div className="max-w-2xl">
            ...
            <h1 className="text-6xl font-black text-primary tracking-tighter leading-none mb-6">Electrical Solutions & Cables</h1>
          ...
          <div className="flex flex-col items-start md:items-end gap-4">
            <div className="bg-surface-container p-6 rounded-sm border-l-4 border-secondary max-w-xs">
          ...
          <Link to={`/products/${product.id}`} key={product.id} className="block group">
            ...
            >
              ...
              <div className="p-8 flex-grow flex flex-col">
                <h3 className="text-2xl font-black text-on-surface mb-3 tracking-tight group-hover:text-primary transition-colors">{product.title}</h3>
                <p className="text-sm text-on-surface-variant mb-6 leading-relaxed line-clamp-3">{product.desc}</p>
                <div className="mt-auto">
                  <span className="block text-[10px] font-bold uppercase tracking-widest text-gray-400 mb-3">Available Variants</span>
                  <p className="text-xs font-medium text-on-surface-variant mb-4">{product.variants.length} products inside</p>
                  <div className="w-full bg-secondary-container text-on-secondary-container py-4 flex items-center justify-center gap-3 font-black text-sm uppercase tracking-widest group-hover:bg-secondary transition-colors">
                    View Specifications
      ...
      <section className="mt-24 bg-primary p-12 md:p-20 relative overflow-hidden">
        <div className="absolute right-0 top-0 w-1/3 h-full opacity-10 pointer-events-none">
          <Download className="w-64 h-64 text-white" />
        </div>
        <div className="relative z-10 max-w-2xl">
          <h2 className="text-4xl md:text-5xl font-black text-white mb-6 tracking-tighter">Bulk Order Requirements?</h2>
          ...
          <div className="flex flex-col sm:flex-row gap-4">
            <Link to="/quote" className="bg-secondary-container flex items-center justify-center text-on-secondary-container px-10 py-4 font-black text-sm uppercase tracking-[0.1em] hover:bg-secondary transition-all">Request a Bulk Quote</Link>
            <button 
              ...
              className="border-2 border-white text-white px-10 py-4 font-black text-sm uppercase tracking-[0.1em] hover:bg-white hover:text-primary transition-all"
            >
              Speak to an Expert
            </button>
          </div>
