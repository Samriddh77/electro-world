...
import { ShieldCheck, Package, Truck, Handshake, ArrowRight, Home as HomeIcon, Factory, Cable, Cpu, Mail, MessageSquare } from 'lucide-react';
  ...
  return (
    <div className="flex flex-col">
      ...
      <section className="relative min-h-[80vh] flex items-center overflow-hidden bg-surface">
        <div className="max-w-7xl mx-auto px-8 w-full grid grid-cols-1 lg:grid-cols-12 gap-12 py-20">
          <motion.div 
          ...
          >
            <div className="inline-block bg-secondary-fixed px-3 py-1 mb-6 w-fit">
              <span className="text-xs font-bold uppercase tracking-[0.2em] text-on-secondary-fixed">Authorized Industrial Distributor</span>
            </div>
            <h1 className="text-6xl md:text-7xl font-black tracking-tighter text-primary leading-[0.9] mb-6">
            ...
            <div className="flex flex-wrap gap-4">
              <Link to="/quote" className="bg-primary text-on-primary px-8 py-4 font-bold text-sm uppercase tracking-widest hover:scale-95 transition-transform duration-150">
                Enquire Now
              </Link>
              <Link to="/products" className="border-2 border-primary text-primary px-8 py-4 font-bold text-sm uppercase tracking-widest hover:bg-surface-container-low transition-colors">
                View Products
              </Link>
            </div>
          </motion.div>
          ...
          >
            ...
            />
            <div className="absolute bottom-0 right-0 bg-primary-container p-6 text-on-primary max-w-[200px]">
        ...
        <div className="max-w-7xl mx-auto px-8">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-12">
            <div className="flex flex-col items-center text-center space-y-4">
              ...
              <h3 className="font-bold text-xs uppercase tracking-widest text-on-surface">Authorized Distributor</h3>
            </div>
            <div className="flex flex-col items-center text-center space-y-4">
              <div className="w-10 h-10 text-primary flex items-center justify-center">
                <Factory className="w-10 h-10" />
              </div>
              <h3 className="font-bold text-xs uppercase tracking-widest text-on-surface">Genuine Products</h3>
            </div>
            <div className="flex flex-col items-center text-center space-y-4">
              <Truck className="w-10 h-10 text-primary" />
              <h3 className="font-bold text-xs uppercase tracking-widest text-on-surface">Same-Day Dispatch</h3>
            </div>
            <div className="flex flex-col items-center text-center space-y-4">
              <Handshake className="w-10 h-10 text-primary" />
              <h3 className="font-bold text-xs uppercase tracking-widest text-on-surface">Trusted Supply Partner</h3>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="py-24 bg-surface">
        <div className="max-w-7xl mx-auto px-8">
          <div className="flex justify-between items-end mb-16">
            <div>
              <h2 className="text-4xl font-black tracking-tighter text-primary">PRODUCT CATEGORIES</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {categories.map((cat, i) => (
              <motion.div 
                ...
                viewport={{ once: true }}
                className="bg-surface-container-lowest p-8 flex flex-col group hover:bg-primary-container transition-colors duration-300"
              >
                <div className="text-primary group-hover:text-on-primary transition-colors">
                  {cat.icon}
                </div>
                <h4 className="mt-8 font-black text-xl tracking-tight text-on-surface group-hover:text-on-primary transition-colors">{cat.title}</h4>
                <p className="mt-4 text-sm text-on-surface-variant group-hover:text-on-primary/80 transition-colors">{cat.desc}</p>
                <Link to={cat.link} className="mt-8 text-[10px] font-bold uppercase tracking-widest text-secondary group-hover:text-white">Learn More</Link>
              </motion.div>
            ))}
          </div>
        </div>
      ...
      <section className="bg-surface-container py-24">
        <div className="max-w-7xl mx-auto px-8">
          <div className="text-center mb-16">
            ...
            <h2 className="text-5xl font-black tracking-tighter text-primary">OUR BRANDS</h2>
          </div>
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-0.5 bg-outline-variant/30">
            {brands.map((brand) => (
              <div key={brand.name} className="bg-surface-container-lowest py-16 flex flex-col items-center justify-center grayscale hover:grayscale-0 transition-all cursor-pointer">
                <span className="text-3xl font-black tracking-tighter" style={{ color: brand.color }}>{brand.name}</span>
                <span className="text-[10px] mt-2 font-bold uppercase tracking-widest opacity-50">{brand.sub}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="py-24 overflow-hidden bg-surface">
        <div className="max-w-7xl mx-auto px-8 grid grid-cols-1 lg:grid-cols-2 gap-20 items-center">
          <div className="relative">
            <div className="absolute -top-10 -left-10 text-[120px] font-black text-surface-container-high -z-10 select-none opacity-20">ELECTRO</div>
            <h2 className="text-4xl font-black tracking-tighter text-primary mb-12">THE INDUSTRIAL ARCHITECT</h2>
            <div className="space-y-10">
              <div className="flex gap-6">
                <div className="flex-shrink-0 w-12 h-12 bg-secondary flex items-center justify-center">
                  <Package className="w-6 h-6 text-on-secondary" />
                </div>
                <div>
                  <h4 className="font-bold text-lg mb-2 uppercase">EXTENSIVE READY STOCK</h4>
              ...
              <div className="flex gap-6">
                <div className="flex-shrink-0 w-12 h-12 bg-secondary flex items-center justify-center">
                  <ShieldCheck className="w-6 h-6 text-on-secondary" />
                </div>
                <div>
                  <h4 className="font-bold text-lg mb-2 uppercase">UNCOMPROMISED QUALITY</h4>
              ...
              <div className="flex gap-6">
                <div className="flex-shrink-0 w-12 h-12 bg-secondary flex items-center justify-center">
                  <MessageSquare className="w-6 h-6 text-on-secondary" />
                </div>
                <div>
                  <h4 className="font-bold text-lg mb-2 uppercase">TECHNICAL ADVISORY</h4>
          ...
          <div className="bg-primary p-12 text-on-primary">
            <h3 className="text-3xl font-black mb-6">READY TO POWER YOUR NEXT PROJECT?</h3>
            <p className="mb-10 text-on-primary/80">Get our latest technical catalogue and price lists directly in your inbox.</p>
